...
InkPanel requests a driving route with `routingPreference: TRAFFIC_AWARE`. Google returns both `duration` (traffic-aware ETA) and `staticDuration`; InkPanel displays the current journey time and the traffic penalty between those values.
...
The e-paper Traffic cell includes visible **Google Maps** attribution because Routes content is being displayed without a corresponding Google map. Keep this attribution intact.
