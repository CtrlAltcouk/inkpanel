...
async function withStudio({ ha = true, prefix = '', profile = MINI } = {}, run) {
  ...
  await store.update('panel-a', { dashboardSections: sections({ type: 'todo', version: 1, config: { listId: '' } }) });
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${server.address().port}${prefix}/harness`);
    await page.waitForFunction(() => window.ready);
    await run({ page, store, calls, sections, recoverDiscovery: () => { discoveryFails = false; } });
  } finally {
    await browser.close();
